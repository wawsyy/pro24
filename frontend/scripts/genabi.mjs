...
const CONTRACT_NAME = "TrustScoreTracker";
...
// If ABI differs, use localhost ABI (for development) and warn
if (deployLocalhost && deploySepolia) {
  if (
    JSON.stringify(deployLocalhost.abi) !== JSON.stringify(deploySepolia.abi)
  ) {
    console.warn(
      `${line}Warning: Deployments on localhost and Sepolia have different ABIs. Using localhost ABI for both networks. Consider re-deploying the contracts on Sepolia to match.${line}`
    );
    // Use localhost ABI for Sepolia as well (development mode)
